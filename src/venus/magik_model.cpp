...
//     success without touching any arguments;
//   - a guarded shim for read_common_param that passes through to the OEM
//     implementation when the param pointer clearly lives in the shared DDR
//     heap, but otherwise returns safe defaults without touching the blob; and
//   - a thin forwarding shim for read_layer_param that records layer headers
//     into g_parsed_layers without changing behaviour.
    ...
    int &index)
{
    ...
    if (param_in_ddr) {
        ...
        if (real) {
            std::fprintf(stderr,
                         "[VENUS] read_common_param shim: delegating to OEM (param=%p ddr_off=%ld index=%d)\n",
                         param, ddr_offset, index);
            std::fflush(stderr);
        ...
        // Fall through to safe stub if dlsym unexpectedly fails.
    }

    // For non-DDR params (or if dlsym fails), return safe defaults and leave
    // `index` unchanged. This avoids dereferencing bogus pointers like 0xd9a.
    layer_name.clear();
    bottoms.clear();
    tops.clear();
    int_params.clear();
    ...
    std::fprintf(stderr,
                 "[VENUS] read_common_param shim: param=%p not in DDR (obj=%s, ddr_base=%p) -> returning defaults, index=%d\n"
                 "  ra0=%p (obj=%s, symbol=%s+0x%lx)\n",
                 param,
                 obj,
                 __ddr_vbase,
...
extern "C" void *memchr(const void *s, int c, size_t n)
{
	...
	/* For performance, the memchr hook is now effectively a thin wrapper
	 * around the real implementation. We keep it in place so it can be
	 * re-armed for diagnostics later, but we no longer perform mincore()
	 * probes or logging on every call.
	 */
 ...
 * suspicious.
 */
// Cap the amount of memcpy logging so we do not flood the serial console or
// spend excessive time in dladdr()/fprintf while still getting a sample of
// interesting traffic.
extern "C" void *memcpy(void *dest, const void *src, size_t n)
{
	/* The global memcpy hook is now a thin, low-overhead wrapper around the
	 * real implementation. We no longer perform logging here to avoid flooding
	 * the console when MagikModelBase::run() performs many small copies.
	 */
	if (!dest || !src || n == 0)
		return dest;
